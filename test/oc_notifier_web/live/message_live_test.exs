defmodule OcNotifierWeb.MessageLiveTest do
  ...
  describe "Index" do
    ...
    test "lists all message", %{conn: conn, message: message} do
      {:ok, _index_live, html} = live(conn, ~p"/message")
    ...
    test "saves new message", %{conn: conn} do
      {:ok, index_live, _html} = live(conn, ~p"/message")

      assert index_live |> element("a", "New Message") |> render_click() =~
               "New Message"

      assert_patch(index_live, ~p"/message/new")
      ...
      assert_patch(index_live, ~p"/message")
    ...
    test "updates message in listing", %{conn: conn, message: message} do
      ...
      assert_patch(index_live, ~p"/message/#{message}/edit")
  ...
  describe "Show" do
    ...
    test "displays message", %{conn: conn, message: message} do
      {:ok, _show_live, html} = live(conn, ~p"/message/#{message}")
    ...
    test "updates message within modal", %{conn: conn, message: message} do
      {:ok, show_live, _html} = live(conn, ~p"/message/#{message}")

      assert show_live |> element("a", "Edit") |> render_click() =~
               "Edit Message"

      assert_patch(show_live, ~p"/message/#{message}/show/edit")
      ...
      assert_patch(show_live, ~p"/message/#{message}")
